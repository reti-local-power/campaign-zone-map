...
# within the study area (Brooklyn) using the cleaned file from
# the previous script
# 
...
#       NOTE: the solar radiation analysis is still a work in progress, I'm
#       hoping for receiving data from Paul Wennberg
...
# + Building footprint 
# note: the file is restricted to buildings with area of 8,000+ sf in boro_1_raw
#       to keep the file size smaller (this shapefile is very large)
bf <- st_read("dat/bldg fp bk/bf.shp") %>%
...
nyserdaurl <- URLencode("https://data.ny.gov/resource/3x8r-34rs.csv?$query=SELECT * 
                        ...
                        county = 'Kings' AND street_address IS NOT NULL
                        LIMIT 1000000")
...
pluto <- read_csv("pluto_24v1.csv",
  ...
  filter(str_sub(bbl, 1, 1) == "3")


# + Solar Radiation Data 
# note: this must be read in from a raw file as I created this in ArcGIS Pro
solrad <- st_read("dat/Solar Power Potential/BK_Suitable_Buildings_Footprint.shp") %>%
